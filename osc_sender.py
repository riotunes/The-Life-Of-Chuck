...
def send_num_stages(
    ...
    ip="127.0.0.1",
    port=9001,
    address="/config/num_stages"
):
    """
    Send OSC message with the number of life stages (music loops) to TouchDesigner.
    This should be sent at startup so TD knows how many stages are in the experience.

    Args:
        num_stages: Number of life stages/music loops
        ip: IP address of TouchDesigner (default: localhost)
        port: OSC port TouchDesigner is listening on (default: 9001)
        address: OSC address/route (default: /config/num_stages)
    """
    try:
        # Create OSC client
        client = udp_client.SimpleUDPClient(ip, port)

        # Send the number of stages
        client.send_message(address, num_stages)
